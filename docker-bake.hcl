...
target "release-s6-overlay" {
    inherits = [ "s6-overlay" ]
    tags = [ 
        "docker.io/just-containers/s6-overlay:${S6_VERSION}",
        "ghcr.io/just-containers/s6-overlay:${S6_VERSION}",
    ]
}
target "release-s6-overlay-symlinks" {
    inherits = [ "s6-overlay-symlinks" ]
    tags = [ 
        "docker.io/just-containers/s6-overlay:${S6_VERSION}-symlinks",
        "ghcr.io/just-containers/s6-overlay:${S6_VERSION}-symlinks",
    ]
}
target "release-s6-overlay-syslogd" {
    inherits = [ "s6-overlay-syslogd" ]
    tags = [ 
        "docker.io/just-containers/s6-overlay:${S6_VERSION}-syslogd",
        "ghcr.io/just-containers/s6-overlay:${S6_VERSION}-syslogd",
    ]
}
...
target "local-s6-overlay" {
    target = "s6-overlay"
    tags = [ "just-containers/s6-overlay:${S6_VERSION}"]
}
target "local-s6-overlay-symlinks" {
    inherits = [ "local-s6-overlay" ]
    target = "s6-overlay-symlinks"
    tags = [ "just-containers/s6-overlay:${S6_VERSION}-symlinks"]
}
target "local-s6-overlay-syslogd" {
    inherits = [ "local-s6-overlay" ]
    target = "s6-overlay-syslogd"
    tags = [ "just-containers/s6-overlay:${S6_VERSION}-syslogd"]
}
